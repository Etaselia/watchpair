...
export function createVoicePeerRecoveryScheduler({
  ...
  return {
    observe(peer, connectionState, recover) {
      if (connectionState !== "disconnected" && connectionState !== "failed") {
        cancel(peer);
        return false;
      }
      if (pending.has(peer)) return false;

      const timer = setTimer(() => {
        pending.delete(peer);
        recover(peer);
      }, connectionState === "failed" ? 0 : disconnectGraceMs);
      pending.set(peer, timer);
      return true;
    },
    cancel,
    dispose() {
